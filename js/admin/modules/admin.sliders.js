    ...
    $.widget("ui.admin_sliders", {
		...
		_create: function () {
			...
			.click(function() {
				...
				var options = $.modules.sliders.options;
				
				for(var i in options) 
					that._options_modules(options, i);
				
				$('<input type="hidden" name="module" value="sliders">').appendTo(that.form);
			...
			.click(function() {
				...
				$('<input type="hidden" name="module" value="sliders">').appendTo(form);
		...
		_update_photo: function() {
			...
			.change(function() {
				...
				for (var i = 0; i < this.files.length; i++) {
					...
					$._upload(current_file,'min',1000,500,that._uploadFile);
				}
			...
			for(var i in that.options.data.image) {
				var div = $('<div class="photo-upload-sliders"></div>').appendTo(that.upload),
					img = that.options.data.image[i];
					
				that._sup_photo(img, div);
					
				$('<div class="mini-upload"></div>').css('background-image', 'url(img/modules/sliders/' + that.options.key + '/' + img + ')').appendTo(div);
			...
			var div = $('<div class="form"></div>').appendTo(form);
			that.submit = $('<button name="submit-photos-sliders" type="submit">Modifier</button>').appendTo(div);
		},
		
		_uploadFile:function(img) {
			var that = this,
				div = $('<div class="photo-upload-sliders"></div>').appendTo('#upload');
